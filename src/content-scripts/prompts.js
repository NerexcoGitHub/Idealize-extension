...
function getMatchingSubCategory(subcategory, objects = window.prompttemplates) {
  let matchingObjects = [];
  console.log(objects);
  for (let i = 0; i < objects.length; i++) {
    if (objects[i].subcategory === subcategory) {
      matchingObjects.push(objects[i]);
    }
  }
  return matchingObjects;
...
function searchAndSubCat(fs){
  let searchTerm = document.querySelector("#search").value;
  let subcategory = document.querySelector("#subCategory").value;
  let prompts = window.prompttemplates;
  console.log(subcategory)
  if (subcategory !==""){
    prompts = getMatchingSubCategory(subcategory, prompts);
  }
  if (searchTerm !== "") {
    prompts = searchPrompts(prompts, searchTerm);
  }
...
function searchResults(fs){
  focusSearch = fs;
  promptTemplateSection.currentPage = 0;
  let searchTerm = document.querySelector("#search").value;
  let prompts = window.prompttemplates;
  let category = document.querySelector("#category").value;
  let subcategoryObject = document.querySelector("#subCategory");
  let subcategory = ""
...
async function insertPromptTemplatesSection(
  ...
  //               <option value="" data-i18n="all_categories">-Categories-</option>
  //               <option value="Copywriting" data-i18n="category_copy_writing">Copywriting</option>
  //               <option value="SEO" data-i18n="category_seo">SEO</option>
  //               <option value="Generative AI" data-i18n="category_generative_ai">Generative AI</option>
  //               <option value="Productivity" data-i18n="category_productivity">Productivity</option>
  //               <option value="Softwares" data-i18n="category_softwares">Softwares</option>
  //           </select>
  //       </div>
  //       <div style="">
  //           <select id="subCategory" class="${css`select`}">
  //               <option value="" data-i18n="sub_categories">Sub Categories</option>'
  ...
  //         searchTerm,
  //       )}&nbsp  &nbsp  → &nbsp &nbsp  ${hs(template.subcategory, searchTerm)} </p>
  ...
  //       <span data-i18n="showing">Showing</span> <span class="${css`paginationNumber`}">${start + 1
  //   }</span> <span data-i18n="to">to</span> <span class="${css`paginationNumber`}">${end}</span> <span data-i18n="of">of</span> <a id="prompt-link"><span class="${css`paginationNumber`}">${templates.length
  ...
  <div>
  <h2 class="${css`h2`}" style="margin-bottom: 5px"><span data-i18n="templates_title">Prompt templates</span></h2> 
  <div class="${css`paginationText`}" id="cgpt-pg-ad"></div>
  <ul class="flex flex-col gap-3.5">
...
`;
  ...
  const subCategories = {
    "Copywriting": ["Writing", "Summarize", "Script Writing", "Product Description", "Marketing"],
    "SEO": ["Keywords", "Research", "Products", "Marketing"],
    "Generative AI": ["Dall-E", "Mid-Journey", "Stable Defusion", "Blue Willow"],
    "Productivity": ["Summarize", "Plan", "Creative Writing", "Problem Solving"],
    "Softwares": ["Web Development", "Backend Development", "Code Generation"]
  };
    ...
    categorySelect.addEventListener("change", function () {
      ...
      subCategorySelect.innerHTML = '<option value="" data-i18n="sub_categories">Sub Categories</option>';

      // Populate sub-category options based on the selected category
...
function updateTemplates(
  ...
  const hs = highlightString;
  let templateHTML = "<h1> No Prompts </h1>";
  ...
  let paginationHTML = `<span data-i18n="showing">Showing</span> <span class="${css`paginationNumber`}">${start + 1
    }</span> <span data-i18n="to">to</span> <span class="${css`paginationNumber`}">${end}</span> <span data-i18n="of">of</span> <a id="prompt-link"><span class="${css`paginationNumber`}">${templates.length
    } <span data-i18n="entries">Entries</span></span></a>`;
