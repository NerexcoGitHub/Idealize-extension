...
const default_prompts = [
  {
    ...
    text: `Give traffic for the keyword{{keyword}} `,
    id: "examplePrompt",
    date: getDate(),
    time: getTime(),
...
function load_prompts(prompts, search = false, search_term = "", tagList = []) {
  ...
  for (let n = prompts.length - 1; n > -1; n--) {
    ...
    let row = template.querySelector(".row1");
    console.log("This is category and subcategory",prompt.category,prompt.subcategory)

    if (prompt.category) {
     const categorySelect = template.querySelector(".selectCategory");
      categorySelect.value = prompt.category;
      console.log("Cat value",categorySelect.value)
    }

    if (prompt.subcategory) {
     
      const subcategorySelect = template.getElementById("subcategoryselect");
      var element = document.createElement("option");
      element.innerText = prompt.subcategory;
      subcategorySelect.append(element);
      // subcategorySelect.value = prompt.subcategory;
      console.log("subcat value",subcategorySelect)

   
    }
    console.log(template.querySelector(".selectCategory"))
    ...
    let categorySelect = row.querySelector(".selectCategory");
    let subCategorySelect = row.querySelector(".selectSubCategory");
    ...
    subCategorySelect.innerHTML = '<option value="" selected data-i18n="subcategory_placeholder">Select SubCategory</option>';
    ...
    if (selectedCategory && subCategories[selectedCategory]) {
      ...
      for (const subCategory of subCategoryOptions) {
        ...
        option.setAttribute("data-i18n",subCategory);
...
Additional information:
    ...
    if (tagList.length === 0 || hasAllTags(prompt.tags, tagList)) {
      ...
      cardContainer.setAttribute("style", "border-radius: 10px;");
...
function toggleTagsEditable(id, row) {
  ...
  if (!tagDiv.classList.contains("editable")) {
    edit_icon.classList.remove("fa-solid fa-pen-nib");
    ...
    edit_icon.classList.add("fa-solid fa-pen-nib");
...
function choose_category(id, row) {
  ...
  let category = row.querySelector(".selectCategory").value;
  chrome.storage.local.get({ prompts: default_prompts }, function (result) {
    let prompts = result.prompts;
    let prompt = getObjectById(id, prompts);
    if (!prompt) {
...
function choose_sub_category(id, row) {
  chrome.storage.local.get({ changedPrompts: [] }, function (result) {
    let changedPrompts = result.changedPrompts;
    if (!changedPrompts.includes(id)) {
      changedPrompts.push(id);
  ...
  let subCategory = row.querySelector(".selectSubCategory").value;
  chrome.storage.local.get({ prompts: default_prompts }, function (result) {
    let prompts = result.prompts;
    let prompt = getObjectById(id, prompts);
    if (!prompt) {
      console.warn(`choose_subcategory: cannot find prompt of id ${id}.`);
      return;
    }
...
function toggle_prompt_editable(id, element, just_title = false) {
  ...
  if (!prompt_text.querySelector("textarea")) {
    ...
    // update buttons
    edit_icon.classList.remove("fa-pen-nib");
    edit_icon.classList.add("fa-floppy-disk");
    // make title editable
    ...
    if (!just_title) {
      ...
      edit_icon.classList.add("fa-pen-nib");
      edit_icon.classList.remove("fa-floppy-disk");
...
function new_prompt(title, text, tags = [], category = "", subcategory = "") {
  let prompt = {
    date: getDate(),
    time: getTime(),
    id: generateUUID(),
...
function category_filter() {
  ...
  if (promptsInCat.length > 0) {
    ...
    main.innerHTML = '<div style="display: flex; justify-content: center; align-items: center; height: 50vh;"><h1>No prompts to display</h1></div>';
